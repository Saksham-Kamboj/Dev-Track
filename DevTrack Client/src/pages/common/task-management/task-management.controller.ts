...
import { getAllTasks, deleteTask, updateTask } from "@/redux/thunks/task.thunks"
...
export const useTaskManagementController = (): TaskManagementControllerResponse => {
  ...
  // Admin-specific handlers
  const handleAssignedToFilter = useCallback((assignedTo: string | null) => {
  ...
  const handleAssignTask = useCallback((taskId: string, assignedTo: string | null) => {
    if (!isAdmin) return
    dispatch(updateTask({ taskId, taskData: { assignedTo: assignedTo || undefined } }))
  }, [dispatch, isAdmin])

  const handleUpdateTaskStatus = useCallback((taskId: string, status: string) => {
    if (!isAdmin) return
    dispatch(updateTask({ taskId, taskData: { status: status as any } }))
  }, [dispatch, isAdmin])

  const handleUpdateTaskPriority = useCallback((taskId: string, priority: string) => {
    if (!isAdmin) return
    dispatch(updateTask({ taskId, taskData: { priority: priority as any } }))
  }, [dispatch, isAdmin])
